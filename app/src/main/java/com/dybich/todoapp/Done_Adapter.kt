...
class Done_Adapter(private val taskList : List<Task>) : RecyclerView.Adapter<My_ViewHolder>() {
            ...
            database.child("users").child(currentUser?.uid.toString()).child("manager").get()
                .addOnSuccessListener { ismanager ->
                    if (ismanager.value.toString() == "true") {
                        ...
                        if (taskName != null) {
                            DoneBottomSheet.newInstance(taskName,"true").show(manager, DoneBottomSheet().tag)
                        }
                    }
